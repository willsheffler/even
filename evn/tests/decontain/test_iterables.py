...
def test_iterables_zipmap():
    zipped = evn.decontain.zipmaps(dict(a=1, b=2), dict(a='a', b='b'))
...
def test_iterables_zipmap_missing():
    zipped = evn.decontain.zipmaps(dict(a=1, b=2), dict(a='a', b='b', c='c'))
    assert isinstance(zipped, dict)
    assert zipped == dict(c=(ipd.dev.NA, 'c'), a=(1, 'a'), b=(2, 'b'))

def test_iterables_zipmap_missing_intersection():
    zipped = evn.decontain.zipmaps(dict(a=1, b=2), dict(a='a', b='b', c='c'), intersection=True)
    assert isinstance(zipped, dict)
    assert zipped == dict(a=(1, 'a'), b=(2, 'b'))

def test_iterables_zipmap_order():
    zipped = evn.decontain.zipmaps(dict(a=2, b=1), dict(a='a', b='b', c='c'), order='val')
    assert isinstance(zipped, dict)
